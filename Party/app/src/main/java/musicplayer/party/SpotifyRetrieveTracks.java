...
public class SpotifyRetrieveTracks extends ActionBarActivity implements Response.Listener,
    ...
    public static final String REQUEST_TAG = "SpotifyRetrieveTracks";
    ...
    private String[] top_tracksname = new String[5]; // array to store top artists retrieved from Spotify web aPI
    private String[] top_tracksID = new String[5];
    private int i=0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        c1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(c1.isChecked())
                {
                    Variable.guest_tracks_preferences[i]= top_tracksID[1];
        ...
        c3.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(c3.isChecked())
                {
                    Variable.guest_tracks_preferences[i]= top_tracksID[2];
                    i++;
                }
            }
        });
        ...
        c4.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(c4.isChecked())
                {
                    Variable.guest_tracks_preferences[i]= top_tracksID[3];
                    i++;
                }
            }
        });
        ...
        c5.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(c5.isChecked())
                {
                    Variable.guest_tracks_preferences[i]= top_tracksID[4];
                    i++;
                }
            }
        });
    ...
    protected void onStart() {
        ...
        mQueue = CustomVolleyRequestQueue.getInstance(this.getApplicationContext())
                .getRequestQueue();
        String url = "https://api.spotify.com/v1/me/top/tracks"; // Spotify web API url to be called to retrieve guest_artists_preferences
    ...
    public void onResponse(Object response) {
        ...
        try {
            ...
            {
                for (int i = 0; i < 5; i++) {
                    top_tracksname[i] = items.getJSONObject(i).getString("name");
                    top_tracksID[i]= items.getJSONObject(i).getString("id");
                }
            }
        ...
        {
            for (int i = 0; i<top_tracksname.length ; i++)
            {

                if (i==0) {
                    c1.setVisibility(View.VISIBLE);
                    c1.setText(top_tracksname[i]);
                }else if (i==1){
                    c2.setVisibility(View.VISIBLE);
                    c2.setText(top_tracksname[i]);
                }
                else if (i==2) {
                    c3.setVisibility(View.VISIBLE);
                    c3.setText(top_tracksname[i]);
                }
                else if (i==3) {
                    c4.setVisibility(View.VISIBLE);
                    c4.setText(top_tracksname[i]);
                }
                else if (i==4) {
                    c5.setVisibility(View.VISIBLE);
                    c5.setText(top_tracksname[i]);
                }
                mTextView.setVisibility(View.VISIBLE);
